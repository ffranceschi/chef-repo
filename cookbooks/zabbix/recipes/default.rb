...
zabbixserver = search(:node, "chef_environment:#{node.chef_environment} AND role:zabbix_server")
if !zabbixserver[0].nil?
  node.normal["zabbix"]["agent"]["servers"] = "#{zabbixserver[0][:fqdn]}"
  node.normal["zabbix"]["web"]["fqdn"] = "#{zabbixserver[0][:fqdn]}" 
end


if node.role?("zabbix_server")
  node.normal["zabbix"]["agent"]["servers"] = "#{node[:fqdn]}"
  if node[:ec2]
    node.normal["zabbix"]["web"]["fqdn"] = "#{node[:ec2][:public_hostname]}"
  else 
    node.normal["zabbix"]["web"]["fqdn"] = "#{node[:fqdn]}"
  end 
end

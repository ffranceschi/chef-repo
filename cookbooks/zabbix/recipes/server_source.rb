...
when "ubuntu","debian"
  ...
  %w{ fping build-essential libmysql++-dev libmysql++3 libcurl3 libiksemel-dev libiksemel3 libsnmp-dev snmp libiksemel-utils libcurl4-openssl-dev }.each do |pck|
    package "#{pck}" do
      action :install
    end
  end
...
when "redhat","centos","scientific"
    ...
    %w{ fping mysql-devel curl-devel iksemel-devel iksemel-utils net-snmp-libs net-snmp-devel openssl-devel redhat-lsb gcc gcc-c++ autoconf automake }.each do |pck|
      package "#{pck}" do
        action :install
      end
    end
